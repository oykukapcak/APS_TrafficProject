...
def generate_routefile(N):
    ...
    with open("data/cross.rou.xml", "w") as routes:
        ...
        <route id="NS1" edges="n12A A2D D2s1" />
        <route id="NS2" edges="n22B B2C C2s2" />
        <route id="WE1" edges="w12A A2B B2e1" />
        <route id="WE2" edges="w22D D2C C2e2" />
        <route id="SN1" edges="s12D D2A A2n1" />
        <route id="SN2" edges="s22C C2B B2n2" />
        <route id="EW1" edges="e12B B2A A2w1" />
        <route id="EW2" edges="e22C C2D D2w2" />
        """, file=routes)
        ...
        for i in range(N):
            ...
            if random.uniform(0, 1) < p_WE:
                print('    <vehicle id="car_%i" type="typeCar" route="WE1" depart="%i"/>' % (vehNr, i), file=routes)
                vehNr += 1
                print('    <vehicle id="car_%i" type="typeCar" route="WE2" depart="%i"/>' % (vehNr, i), file=routes)
                vehNr += 1

            if random.uniform(0, 1) < p_EW:
                print('    <vehicle id="car_%i" type="typeCar" route="EW1" depart="%i"/>' % (vehNr, i), file=routes)
                vehNr += 1
                print('    <vehicle id="car_%i" type="typeCar" route="EW2" depart="%i"/>' % (vehNr, i), file=routes)
                vehNr += 1
